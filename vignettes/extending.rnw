...
\item[7:] Compute the basis for non-estimable functions. This applies only when there is a possibility of rank deficiency in the model, and "lqs" methods cannot handle that. All linear functions are estimable, and we signal that by setting "nbasis" equal to a $1\times1$ matrix of "NA". If rank deficiency were possible, thje \pkg{estimability} package (which is required by \lsm{}) provides a "nonest.basis" function that makes this fairly painless---I would have coded:
<<eval=FALSE>>=
