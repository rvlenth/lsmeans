...
Related (in a surprising way) to the above, if you check for any error conditions in "recover.data", simply have it return a character string with the desired message, rather than invoking "stop". This provides a cleaner exit. The reason is that whenever "recover.data" throws an error, an informative message suggesting that "data" be provided is displayed. But a character return value is tested for an throws a different error with your message.
