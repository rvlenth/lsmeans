...
<<>>=
test(pairs(lsmeans(housing.plr, ~ Infl | Type)), joint = TRUE)
@
...
<<>>=
test(pairs(lsmeans(housing.plr, ~Cont | Type)), joint = TRUE)
@
So the effect is inconclusive for both atria and terraces.

The location and scale of the latent variable are not identifiable, and below we use "rescale" to change the reference scale to center on 50 with an error SD around 10.
<<>>=
lsmeans(housing.plr, ~ Infl | Type, rescale = c(50,10))
@


%%%%estimating thresholds for the b, in that an extra factor (named \dqt{cut} by default) is created to identify which boundary between scale positions we wish to use in predictions. 
