...
The \lsm{} package provides a "plot" method for "lsmobj" objects that displays confidence intervals for the LS~means. Its use requires that the \pkg{lattice} package \citep{latti} to be installed. Optional arguments are passed to the "update" method for the object (allowing changes to confidence levels, adjustments, etc.) and to \pkg{lattice}'s "xyplot" function (allowing changes to colors, layout, etc.)
...
To illustrate these capabilities, consider the first model we fitted to the "Oats" data. In the following code we include arguments that affect both the values displayed and the layout of the plot:
<<OatsCIs, fig=TRUE, height=3.5>>=
plot(lsmeans(Oats.lmer, ~ nitro | Variety), 
     type="response", level=.90, layout = c(3,1))
@

In addition, let's do a graphical comparison of the two models we have fitted to the "Oats" data. 
