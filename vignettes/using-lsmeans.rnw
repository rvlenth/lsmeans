...
\section{Displaying LS means graphically}\ixsub{graphical displays}{interaction plot}
...
Chick.lmer <- lmer(weight ~ Diet * Time + (0 + Time | Chick), 
    data = ChickWeight)
@
We can then call \wixcode{lstrends} to estimate and compare the average slopes for each diet.
<<>>=
