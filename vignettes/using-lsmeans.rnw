...
\title{Least-squares Means: The R Package \lsm{}} %% without formatting
...
There are a few other \R{} packages that provide capabilities that overlap with those of \lsm{}. The \pkg{effects} package \citep{effects,fox09} can compute LS~means. However, an important distinction is that for unbalanced dataset, it produces weighted averages of predictions. For example, with the nutrition example, consider
<<>>=
library("effects")
nutr.eff <- Effect(c("race"), nutr.lm)
as.data.frame(nutr.eff)
@
The results in the first and third rows are the same as those obtained previously in "lsmeans" with "weights="\dqt{outer}---not the equally weighted LS~means.
However, "Effects" does not do an estimability check, and its results for Hispanics are suspect.

The emphasis of \pkg{effects} is on graphical rather than tabular displays. 
For a model with continuous predictors, \pkg{effects} automatically generates a grid of values of those predictors for use in its graphs, whereas one would have to use an "at" specification to do the same in \lsm{}. Thus, \pkg{effects} has special strengths for curve-fitting models such as splines. In contrast, \lsm{}'s strengths are more in the area of factorial models where one wants traditional summaries in the form of estimates, contrasts, and interaction plots.
