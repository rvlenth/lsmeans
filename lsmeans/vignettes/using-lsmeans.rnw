...
This kind of automatic support is available for most popular response transformations such as "log", "log10", "asin(sqrt())", and many others. The Details section for \verb|help("make.tran")| provides a complete list. It is also possible to support custom transformations via the "tran" argument in the "update" method---see its help page.

\subsection{Using \code{make.tran}}
The "make.tran" function provides support for yet more popular types of transformations, especially those that require specifying one or more parameters. Examples are general power transformations, Box-Cox transformations, and transformations with a shifted origin such as "log(y + a)".
