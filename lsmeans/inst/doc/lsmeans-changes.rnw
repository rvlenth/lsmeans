...
\usepackage[margin=1in, fancyvrb,unnumbered]{rvl}

\def\pkg#1{\textsf{#1}}
\def\lsm{\pkg{lsmeans}}
\def\code{\texttt}

\title{Redesign of \lsm}
...
@
The "glhargs" capability is replaced by a method for "glht" in the \pkg{multcomp} package:
<<>>=
...
Most other methods are S3 ones, as those are suitable to our needs and often extend existing S3 methods. The "glht" methodis illustrated earlier in this document. The "confint" and "test" methods are really courtesy methods for "summary" with argument "infer" set to "c(TRUE,FALSE)" and "c(FALSE,TRUE)" respectively.
